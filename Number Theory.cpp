...
//http://www.geeksforgeeks.org/how-to-avoid-overflow-in-modular-multiplication/  refer this for understanding...
...
int phi(const int n)
{
        ...
        ++p )
  {
   ...
   // Used to compute phi(i) in less time than the previous one..  reference GeeksforGeeks...
void computeTotient()
{
    ...
    for (long int p=2; p<=10000000; p++)
    {
        ...
        if (phi[p] == p)
        {
            ...
            // multiples of p
            for (long int i = 2*p; i<=10000000; i += p)
            {
...
template<typename T>
void factor(T N, factors &facts) 
{  //factors N into prime factors in facts
    const int mp = 100;
